...
if __name__ == '__main__':
    rospy.init_node('item4')
    turtle_vel = rospy.Publisher('/tb3/cmd_vel', Twist, queue_size=1)
    rate = rospy.Rate(Friquent)  # 循环执行，更新频率是10hz

    while not rospy.is_shutdown():
        msg = Twist()
        (roll, pitch, yaw) = euler_from_quaternion([xx4, yy4, zz4, ww4])
        # chanange xyz
        yaw = -yaw
        yaw = yaw + math.pi
        ...
        D_err = math.sqrt(math.pow((X_t - x4), 2) + math.pow((Y_t - y4), 2))
        u = D_err

        if (Y_t - y4) == 0 and (X_t - x4) > 0:
            yaw_t = 0
        if (Y_t - y4) > 0 and (X_t - x4) > 0:
            yaw_t = math.atan((Y_t - y4) / (X_t - x4))
        if (Y_t - y4) > 0 and (X_t - x4) == 0:
            yaw_t = 0.5 * math.pi
        if (Y_t - y4) > 0 and (X_t - x4) < 0:
            yaw_t = math.atan((Y_t - y4) / (X_t - x4)) + math.pi
        if (Y_t - y4) == 0 and (X_t - x4) < 0:
            yaw_t = math.pi
        if (Y_t - y4) < 0 and (X_t - x4) < 0:
            yaw_t = math.atan((Y_t - y4) / (X_t - x4)) + math.pi
        if (Y_t - y4) < 0 and (X_t - x4) == 0:
            yaw_t = 1.5 * math.pi
        if (Y_t - y4) < 0 and (X_t - x4) > 0:
            yaw_t = math.atan((Y_t - y4) / (X_t - x4)) + 2 * math.pi
